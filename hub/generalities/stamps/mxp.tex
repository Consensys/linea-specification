...
\end{enumerate}
In other words, the only time the \mxpStamp{} \emph{may} change is when dealing with a memory expansion inducing instruction that isn't squashed by a \suxSH{} or \soxSH{}. The precise conditions which lead to an increment in $\mxpStamp$ will be dealt with in a case by case fashion in section~\ref{hub: instruction handling}.
