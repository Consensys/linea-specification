...
\begin{multicols}{3}
	\begin{enumerate}
		...
		\item $\order{\accTrmIsPrecompile}$
			%
		\item $\order{\accDelegationAddressHi}$
		\item $\order{\accDelegationAddressLo}$
		\item $\order{\accDelegationAddressHiNew}$
		\item $\order{\accDelegationAddressLoNew}$
		\item $\order{\accDelegationNumber}$
		\item $\order{\accDelegationNumberNew}$
		\item $\order{\accIsDelegated}$
		\item $\order{\accIsDelegatedNew}$
		% \item[\vspace{\fill}]
	\end{enumerate}
\end{multicols}
