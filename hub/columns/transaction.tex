...
(\emph{b}) that the nonce provided in the transaction coincides with that of the sender account as found in the state.
...
\begin{enumerate}
	...
	% % DEPRECATED in favor of \txTypeSupportsEipFifteenFiftyNineGasSemantics
	% \item
	% 	\godGiven{}
	% 	$\txIsTypeTwo$:
	...
	\item
		\godGiven{}
		$\txTypeSupportsEipFifteenFiftyNineGasSemantics$
		binary column which equals $1$ for transactions supporting fee market pricing,
		i.e. \cite{EIP-1559} gas semantics;
	\item
		\godGiven{}
		$\txTypeSupportsDelegationLists$
		binary column which equals $1$ for transactions supporting delegation lists;
	\item
		\godGiven{}
		$\txGasLimit$:
	...
	\item
		\godGiven{}
		$\txInitialGas$:
		initial gas provided to execution;
	\item
		\godGiven{}
		$\txGasPrice$:
...
see section~(\ref{txn data: processing: sysi transaction processing})
\begin{enumerate}[resume]
	...
	\item
		...
		boolean column;
		lights up for \cite{EIP-2935} system transactions;
	\item
		\godGiven{}
		$\txnSystemTransactionDataCol{k}$, $1 \leq k \leq \txnDataHubNumberOfDataColumns$:
		data column used by system transactions;
\end{enumerate}
The following columns are related to \cite{EIP-7702}.
\begin{enumerate}[resume]
	\item
		\markAsJustifiedHere{}
		$\txLengthOfDelegationList$:
		counts how many delegation tuples are dealt with in the \hubMod{} module;
	\item
		\markAsJustifiedHere{}
		$\txNumberOfSuccessfulSenderDelegations$:
		counts how many \textbf{valid} delegation tuples \loc{tuple} have the property
		that the authority coincides with the \texttt{from} field of the transaction;
\end{enumerate}
\saNote{}
\txLengthOfDelegationList{}
is needed in the \txnDataMod{} module
to compute the upfront gas cost of
\textbf{set code transactions}
i.e. \textbf{type 4 transactions}.

\saNote{}
\txNumberOfSuccessfulSenderDelegations{}
is needed by the \txnDataMod{} module
to compare the sender nonce
(as read off of the sender account in the \hubMod{} module
and recorded in the \txNonce{} column)
to the transaction nonce.
