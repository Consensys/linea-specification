...
\begin{enumerate}
	...
	\item
		...
		index identifying authority tuples of transactions supporting authority tuples;
		starts counting at $1$ and resets with every transaction;
	\item
		\authAuthorityEcrecoverSuccess{}:
	...
	\item
		\authSenderIsAuthority{}:
		binary column that lights up on every authoritization tuple where
		the authority was recovered and the authority coincides with the
		transaction sender;
	\item
		\authSenderIsAuthorityAcc{}:
		accumulator column that tallies the number of authorization tuples that are both
		valid and have as authority the sender of the transaction;
	\item
	...
	\item
		\authAuthorityNonce{}:
		column containing the nonce of the authority address as read off of the state
		\emph{provided} the authority address was successfully \macroEcrecover{}'ed;
	\item
		\authAuthorityHasEmptyCodeOrIsDelegated{}:
		binary column;
	...
	\item
		\authAuthorizationTupleIsValid{}:
		binary column;
		indicates whether the authorization tuple is valid;
		validity of the tuple dictates whether the authority account is modified or not;
	\item
...
\[
	\mathbf{c} \equiv
	\begin{cases}
		\If \authDelegationAddressIsZero \equiv \false: & \texttt{0x\,ef\,01\,00} \cdot \col{delegation\_address} \\
