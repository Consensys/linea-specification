...
\begin{enumerate}[resume]
	...
	\item \TXWARM{}:
		binary column which is on \emph{if and only if} a transaction is currently in the prewarming phase; abbreviated to \txWarm{};
	\item \TXAUTH{}:
		binary column which is on \emph{if and only if} a transaction is currently in the delegation phase; abbreviated to \txAuth{};
	\item \TXINIT{}:
		binary column which is on \emph{if and only if} a transaction is currently in the initialization phase; abbreviated to \txInit{};
	\item \TXEXEC{}:
		binary column which is on \emph{if and only if} a transaction is currently in the execution phase; abbreviated to \txExec{};
	\item \TXFINL{}:
		binary column which is on \emph{if and only if} a transaction is currently in the finalization phase; abbreviated to \txFinl{};
\end{enumerate}
Rows $i$ satisfying $\txSkip_{i} \equiv 1$ are called \textbf{skipping rows}.
We similarly define
\textbf{prewarming     rows} ($\txWarm _{i} \equiv 1$),
\textbf{authorization  rows} ($\txAuth _{i} \equiv 1$),
\textbf{initialization rows} ($\txInit _{i} \equiv 1$),
\textbf{execution      rows} ($\txExec _{i} \equiv 1$) and
\textbf{finalization   rows} ($\txFinl _{i} \equiv 1$).
...
\end{enumerate}
We will impose that
$\cnWillRev = 1$ \emph{iff} the present execution context reverts, for one reason or another;
$\cnGetsRev = 1$ \emph{iff} the parent execution context reverts;
$\cnSelfRev = 1$ \emph{iff} the present execution context reverts through its own fault (either by encountering an exception or successfully executing on a \inst{REVERT}). We will impose the following constraints:
