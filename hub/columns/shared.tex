...
\begin{enumerate}[resume]
	\item \TXSKIPSTD{}:
		binary column which is on \emph{if and only if} a transaction processing requires no code execution; abbreviated to \txSkipStd{};
	\item \TXSKIPPRC{}:
		binary column which is on \emph{if and only if} a transaction processing requires no code execution; abbreviated to \txSkipPrc{};
	\item \TXWARM{}:
