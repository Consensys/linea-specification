...
For instance the \inst{MODEXP} precompile uses these fields to transfer various values from \textsc{ram} to the \hubMod{} for further processing
(e.g. byte size parameters $\locBase$, $\locExponent$, $\locModulus$ but also the leading word of the exponent.)
Similarly the \inst{BLAKE} precompile uses these fields to transit
