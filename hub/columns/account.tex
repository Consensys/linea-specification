...
\end{enumerate}
Recall that an address $\textsf{a}$ is deemed to be \textbf{non-existent} in the world state $\sigma$ if % \ob{TODO: What's the number?}
...
\begin{enumerate}[resume]
	...
	\item $\accRlpAddrRecipe$:
		``recipe'' column which distinguishes between the two ``recipes'' for computing a deployment address; % \ob{TODO: add reference to \rlpAddrMod{} module chapter}
