...
\end{enumerate}
Recall that, as in \cite{EIP-161}, an address $\textsf{a}$ is deemed to be \textbf{non-existent} in the world state $\sigma$ if
...
\begin{enumerate}[resume]
	\item $\accMarkedForDeletion$ and $\accMarkedForDeletion\new$:
		binary columns;
		track whether an address was successfully marked for selfdestruction within a given transaction;
\end{enumerate}
The following columns are \textsc{zk-evm} specific columns. % \ob{TODO: write section and point to it}
\begin{enumerate}[resume]
...
\begin{enumerate}[resume]
	...
	\item $\accRlpAddrRecipe$:
		``recipe'' column which distinguishes between the two ``recipes'' for computing a deployment address, see chapter~(\ref{chap: addr rlp});
...
\begin{enumerate}[resume]
	...
	\item
		\accDelegationAddressHiNew{} and \accDelegationAddressLoNew{}:
		columns containing the updated delegation address of the account;
	\item
		\accDelegationNumber{} and \accDelegationNumberNew{}:
		columns containing the current delegation address of the account;
	\item
		\accCheckForDelegation{} and \accCheckForDelegationNew{}:
		binary columns indicating whether it is necessary to check for delegation;
	\item
...
see section~(\ref{hub: account: account delegation}) and specifically
