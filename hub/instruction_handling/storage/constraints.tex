...
\begin{description}
	...
	\item[\underline{Setting $\nonStackRows$ and peeking flags:}]
		...
		\item[\underline{The first context-row:}]
			we \textbf{unconditionally} impose that
			\[
...
\begin{description}
	...
	\item[\underline{Setting the refund:}]
		...
		see section~(\ref{hub: generalities: refunds: EXEC phase: instructions that aren't SSTORE don't accrue refunds}).
		Futhermore, refunds are only granted in non-reverting contexts ($\cnWillRev \equiv 0$),
		see section~(\ref{hub: generalities: refunds: EXEC phase: refunds aren't accrued if the frame reverts}).
