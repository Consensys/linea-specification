	...
	\item[\underline{\underline{Setting $\nonStackRows$ and peeking flags:}}]
		...
		\begin{enumerate}
			...
			\item \If $\scenSelfdestructWontRevertNotYetMarked_{i} = 1$ \Then
				...
				\]
		\end{enumerate}
		\saNote{} For instructions raising the \haltFlag{} it holds that $\cmc \equiv 1$, see section~\ref{hub: generalities: context: cmc flag}.
			...
			\item[\underline{The unexceptional case:}]
				...
				\begin{description}
					...
					\item[\underline{The account did not have code prior to the transaction:}]
							\If
							\[
								\locAccountHadNoCodeInitially = 1
							\]
							\Then
						\begin{description}
							\item[\underline{}]
