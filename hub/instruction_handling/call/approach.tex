...
and abusing terminology somewhat,
we define
\begin{description}
	\item[\underline{\underline{Externally owned accounts:}}]
		defined as accounts with
		one of the following:
		\begin{description}
			\item[\underline{``True \textsc{eoa}'s'':}]
				these have are characterized by
				\begin{itemize}
					\item having empty code ($\accHasCode \equiv \false$) and
					\item not being a precompile ($\accTrmIsPrecompile \equiv \false$)
				\end{itemize}
			\item[\underline{Delegated accounts pointing to empty code:}]
				these have are characterized by
				\begin{itemize}
					\item having nonempty code ($\accHasCode \equiv \true$)
					\item and being delegated ($\accIsDelegated \equiv \true$)
					\item where the delegate has empty byte code ($\accHasCode \equiv \false$)
				\end{itemize}
			% \item[\underline{Delegated accounts pointing to a delegated account:}]
			% 	with as delegate an account which itself either
			% 	\begin{itemize}
			% 		\item having nonempty code ($\accHasCode \equiv \true$)
			% 		\item and being delegated ($\accIsDelegated \equiv \true$)
			% 		\item where the delegate
			% 			\begin{itemize}
			% 				\item has nonempty byte code ($\accHasCode \equiv \true$)
			% 				\item and is delegated ($\accIsDelegated \equiv \true$)
			% 			\end{itemize}
			% 	\end{itemize}
		\end{description}
	\item[\underline{\underline{Smart contracts:}}]
		defined as accounts with
		one of the following:
		\begin{description}
			\item[\underline{``True \textsc{smc}'s'':}]
				these have are characterized by the conjunction of
				\begin{itemize}
					\item having nonempty code ($\accHasCode \equiv \true$)
					\item not being delegated ($\accIsDelegated \equiv \false$)
				\end{itemize}
			\item[\underline{Delegated accounts pointing to nonempty code:}]
				these have are characterized by the conjunction of
				\begin{itemize}
					% \item having nonempty code ($\accHasCode \equiv \true$, (\sanityCheck))
					\item being delegated ($\accIsDelegated \equiv \true$)
					\item the delegate having nonempty byte code ($\accHasCode \equiv \true$)
						% \begin{itemize}
						% 	\item has nonempty byte code ($\accHasCode \equiv \true$)
						% 	\item and isn't delegated ($\accIsDelegated \equiv \false$)
						% \end{itemize}
				\end{itemize}
		\end{description}
	\item[\underline{\underline{Precompiles:}}]
		defined as accounts
		\begin{itemize}
			\item having empty code ($\accHasCode \equiv \false$) and
			\item raising the precompile flag (i.e. $\accTrmIsPrecompile \equiv \true$)
		\end{itemize}
\end{description}
