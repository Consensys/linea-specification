		...
		\item[\underline{\underline{Miscellaneous-row $n^°(i + \prcStandardSuccessThirdMiscRowOffset)$:}}]
			...
			\begin{description}
				\item[\underline{Setting lookup flags:}]
					...
					\]
			\end{description}
			\saNote{} In other words the ``result transfer'' step of a call to the \instEcadd{}, \instEcmul{}, \instEcpairing{} is only required if the \inst{CALL} is provided with nonzero \RAC{}.
			\begin{description}
