	...
	\item[\underline{\underline{Miscellaneous row $n^°(i + \prcCommonMiscRowOffset)$:}}]
		...
		\begin{description}
			...
			\item[\underline{Defining \oobMod{} shorthands:}] 
				...
				\saNote{} \label{hub: instruction handling: call: precompiles: common: generalities: sanity checks for BLS precompiles and P256_Verify}
				\If $\scenPrecompileCommonBls _{i} = 1$ or $\scenPVerify _{i} = 1$ \Then one expects
				\begin{enumerate}
			...
			\item[\underline{Some shorthands for elliptic curve precompiles:}]
				...
				\begin{description}
					...
					\item[\underline{\instEcadd{}, \instEcmul{} and \instEcpairing{} as well as \inst{BLS}-precompile and \instPVerify{} specific:}]
						we set
						\[
							\left\{ \begin{array}{lcl}
