...
\instBlsPairingCheck{},
\instBlsMapFpToGOne{},
\instBlsMapFpTwoToGTwo{} or
\instPVerify{} precompile.
...
\begin{description}
	\item[\underline{\underline{Miscellaneous row $n^°(i + \prcCommonMiscRowOffset)$:}}]
		...
		\begin{description}
			...
			\item[\underline{Setting \oobMod{} instruction:}] 
				...
				\[
					...
					\left[ \begin{array}{crcl}
						...
						+ & \oobInstPVerify           & \cdot & \scenPVerify           _{i} \\
			...
			\item[\underline{Formal properties of these \oobMod{}-shorthands:}] 
				we assert the following
				\begin{enumerate}
					\item\label{hub: instruction handling: call: precompiles: common: generalities: oob shorthands are provably binary}
						by construction and by the computation in the \oobMod{} module it holds that
						\[
							\left\{ \begin{array}{lclr}
					...
					\item unconditional exclusivity
						\[
							\locOobResultExtractCallData \cdot \locOobResultEmptyCallData \equiv 0
							\quad (\sanityCheck)
						\]
					\item\label{hub: instruction handling: call: precompiles: common: generalities: sanity checks for BLS precompiles and P256 Verify}
						\If
						\[
							\left[ \begin{array}{lr}
						...
						\Then
						\[
							\locOobResultEmptyCallData = 0 \quad (\sanityCheck)
						\]
					\item\label{hub: instruction handling: call: precompiles: common: generalities: sanity checks for common prcs other than P256 Verify}
						\If
						\[
							\left[ \begin{array}{lr}
								+ & \scenPrecompileCommonLondon _{i} \\
						...
						\Then
						\[
							\locOobResultExtractCallData + \locOobResultEmptyCallData = \locOobResultHubSuccess
							\quad (\sanityCheck)
						\]
						\saNote{}
						The above amounds to $\locOobResultExtractCallData \vee \locOobResultEmptyCallData \iff \locOobResultHubSuccess$.
					\item\label{hub: instruction handling: call: precompiles: common: generalities: sanity checks for P256 Verify}
						\If $\scenPrecompileCommonOsaka  _{i} = \true$ \Then
						\[
							\If     \locOobResultExtractCallData = 1 ~ \Then \locOobResultHubSuccess = 1
							\quad (\sanityCheck)
						\]
						\saNote{}
						The above amounds to $\locOobResultExtractCallData \vee \locOobResultEmptyCallData \implies \locOobResultHubSuccess$.
				\end{enumerate}
				see note~(\ref{hub: instruction handling: call: precompiles: common: generalities: empty call data in well formed data shorthand for BLS precompiles and P256Verify})
			...
			\item[\underline{Some shorthands for elliptic curve precompiles:}]
				...
				\begin{description}
					\item[\underline{\instEcrecover{} specific:}]
						...
						\saNote{}\label{hub: instruction handling: call: precompiles: common: generalities: ecrecover recovery reminder that MMU success bit is a prediction}
						We finish by reminding that from the point of view of the \hubMod{}
					...
					\item[\underline{\instEcadd{}, \instEcmul{} and \instEcpairing{} as well as \inst{BLS}-precompile specific:}]
						...
						as well as the \inst{BLS}-precompiles (i.e. the precompiles whose flags go into the \scenPrecompileCommonBls{} shorthand) and \instPVerify{}.

						\saNote{}\label{hub: instruction handling: call: precompiles: common: generalities: empty call data in well formed data shorthand for BLS precompiles and P256Verify}
						Our definition of the \locWellFormedData{} shorthand may seem blatantly wrong in the case of
						\inst{BLS}-precompiles, as it implies in particular that
						$\locWellFormedData \equiv \true$ whenever $\locOobResultEmptyCallData \equiv \true$.
						If this were the case, it would explicitly contradict the specifications of \instPointEvaluation{} and the \inst{BLS}-precompiles,
						which are all required to throw an exception when called with empty call data.
						Surely one must insist on having $\locWellFormedData{} \equiv \false$ in those cases.
						Despite appearances, this will actually be the case.
						Indeed the \oobMod{}-module \emph{always} outputs $\locOobResultEmptyCallData \equiv \false$ for these precompiles,
						even when their call data happens to be empty,
						see section~(\ref{oob: precompiles: common precompiles: generalities}).
						Thus, the inclusion of \locOobResultEmptyCallData{} as a subcase of \locWellFormedData{} is unproblematic for these precompiles.
						...
						\end{center}
						\saNote{}
						We have, by construction, the following relations:
						\[
							\left\{ \begin{array}{lclr}
							...
							\end{array} \right.
						\]
					\item[\underline{\instPVerify{} specific:}]
						we set
						\[
							\left\{ \begin{array}{lcl}
								\locOobPVerifySufficientGasWrongCds & \define & \locOobResultHubSuccess - \locOobResultExtractCallData                                    \\
								\locSignatureVerificationFailure    & \define & \locOobResultExtractCallData \cdot (1 - \miscMmuSuccessBit_{i + \prcCommonMiscRowOffset}) \\
								\locSignatureVerificationSuccess    & \define & \locOobResultExtractCallData \cdot \miscMmuSuccessBit_{i + \prcCommonMiscRowOffset}       \\
								% \locSignatureVerificationFailure    & \define &
								% \left[ \begin{array}{cl}
								% 	+ & \locOobPVerifySufficientGasWrongCds                                                       \\
								% 	+ & \locOobResultExtractCallData \cdot (1 - \miscMmuSuccessBit_{i + \prcCommonMiscRowOffset}) \\
								% \end{array} \right] \\
							\end{array} \right.
						\]
						\saNote{} \label{hub: instruction handling: call: precompiles: common: generalities: ecrecover recovery success failure flags are provably binary}
						These bits will be used in the processing of \instPVerify{}.
						In that context (and given the precompile succedes)
						\begin{itemize}
							\item
								$\locOobPVerifySufficientGasWrongCds \equiv 1$
								precisely when the precompile is given sufficient gas but its call data has the wrong size ($\neq 160$);
								the \mmuMod{} module does not get called;
								the underlying \inst{CALL}-type instruction succeeds;
								the return data is empty, i.e. $\textbf{o} = ()$;
							\item
								$\locAddressRecoveryFailure \equiv 1$
								precisely when the precompile is given sufficient gas and its call data has the right size ($= 160$);
								the \mmuMod{} module gets called but signature verification fails;
								the underlying \inst{CALL}-type instruction succeeds;
								return data is empty, i.e. $\textbf{o} = ()$;
							\item
								$\locAddressRecoverySuccess \equiv 1$
								precisely when the \mmuMod{} module gets called
								and signature verification succeeds;
								the underlying \inst{CALL}-type instruction succeeds;
								return data is nonempty;
								actually $\textbf{o} \equiv \utt{00}\, \utt{00}\, \cdots \, \utt{01} \in \mathbb{B}_{\evmWordSize}$);
						\end{itemize}

						\saNote{}
						We have, by construction, and given that the precompile at hand is \instPVerify{}, the following relations:
						\[
							\left\{ \begin{array}{lclr}
								\locOobPVerifySufficientGasWrongCds & \equiv & \text{provably binary} & \quad (\trash) \\
								\locSignatureVerificationFailure    & \equiv & \text{provably binary} & \quad (\trash) \\
								\locSignatureVerificationSuccess    & \equiv & \text{provably binary} & \quad (\trash) \vspace{2mm} \\
								\left[ \begin{array}{cl}
									+ \locOobPVerifySufficientGasWrongCds \\
								        + \locSignatureVerificationFailure    \\
									+ \locSignatureVerificationSuccess    \\
								\end{array} \right] & = &  \locOobResultHubSuccess \quad (\trash) \\
							\end{array} \right.
						\]
						\saNote{}
						The same comment about $\miscMmuSuccessBit_{i + \prcCommonMiscRowOffset}$
						being a \hubMod{}-module prediction applies as in
						note~(\ref{hub: instruction handling: call: precompiles: common: generalities: ecrecover recovery reminder that MMU success bit is a prediction}).
				\end{description}
