	...
	\item[\underline{\underline{Miscellaneous row $n^°(i + \prcCommonMiscRowOffset)$:}}]
		...
		\begin{description}
			...
			\item[\underline{Defining \oobMod{} shorthands:}] 
				...
				\end{enumerate}
				see note~(\ref{hub: instruction handling: call: precompiles: common: generalities: empty call data not well formed for BLS precompiles and P256_Verify})
			...
			\item[\underline{Some shorthands for elliptic curve precompiles:}]
				...
				\begin{description}
					...
					\item[\underline{\instEcadd{}, \instEcmul{} and \instEcpairing{} as well as \inst{BLS}-precompile and \instPVerify{} specific:}]
						...
						\saNote{} \label{hub: instruction handling: call: precompiles: common: generalities: empty call data not well formed for BLS precompiles and P256_Verify}
						\inst{BLS}-precompiles and \instPVerify{} disallow empty call data. The above is however not an issue: for \inst{BLS}-precompiles and \instPVerify{}, \locOobResultEmptyCallData{} is implicitely 
						filtering out the case of empty call data (see section \ref{}). Hence, empty call data is not a case of \locWellFormedData{} for these precompiles.
