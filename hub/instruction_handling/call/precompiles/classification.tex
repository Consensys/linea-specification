...
In particular we explain which precompiles may raise the $\scenPrcFailureKnownToRam$ flag.
	\begin{figure}[!h]
		...
		\instPointEvaluation{}            & insufficient gas                               & \multirow{2}{*}{\toOob}   & malformed data                                                    & \multirow{2}{*}{\text{\cite{EIP-4844}, Cancun}}  \\
                                                  & $\vee~\cds \neq 192$                           &                           &                                                                   &                                                  \\ \hline \hline
		...
		\instBlsMapFpTwoToGTwo{}          & insufficient gas                               & \multirow{2}{*}{\toOob}   & malformed data                                                    &                                                  \\
                                                  & $\vee~\cds \neq 128$                           &                           &                                                                   &                                                  \\ \hline \hline
			\instPVerify{}                    & insufficient gas                               & \toOob                    & $\nothing$                                                        & \multirow{2}{*}{\text{\cite{EIP-7951}, Osaka}}   \\ \hline
		\end{tabular}
		}
...
\begin{description}
	\item[\instEcrecover{}:]
		...
		gas cost is fixed;
		the precompile may however return either $() \in \mathbb{B}_{0}$ if data is malformed or no public address is recoverable;
		otherwise it returns $\textbf{o} \in \mathbb{B}_{\evmWordSize}$ a $\evmWordSize$ byte slice containing a $\addressSize$ byte address. 
	...
	\item[\instModexp{}:]
		...
		furthermore extracting some of these parameters (e.g. $\locExponentLogEYP$) requires previously having extracted other parameters (namely $\locBase$ and $\locModulus$);
	...
	\item[\instEcadd{} and \instEcmul{}:]
		...
		if none of these conditions are triggered the output is a \evmWordSize{}-byte integer slice $\textbf{o} \in \mathbb{B}_{\evmWordSize}$;
...
\noindent The next precompile was introduced in the \textsc{Cancun} hardfork, specifically with \cite{EIP-4844};
\begin{description}
...
\noindent The next precompile was introduced in the \textsc{Osaka} hardfork, specifically with \cite{EIP-7951}.
It behaves near identical with the \instEcrecover{} precompile.
In particular, and contrary to all precompiles introduced since the \textsc{Cancun} hardfork,
failure (of the underlying \inst{CALL}-type instruction) cannot be triggered by either
$\CDS$ being wrong (i.e. $\neq 160$)
nor by the contents of call data being malformed
(as measured by the \ecDataMod{}-module).
\begin{description}
	\item[\instPVerify{}:]
		can fail due to:
		\locBlsPrecompileFailureReasonGas;
		as for \instEcrecover{}, failure is entirely detectable using \hubMod{} data
		and entirely comes down to insufficient gas;
		gas cost is fixed;
		the precompile returns $() \in \mathbb{B}_{0}$ in any of the following scenarios:
		(\emph{a}) if call data size is $\neq 160$,
		(\emph{b}) if call data contents are malformed (as measured by the \ecDataMod{}-module),
		(\emph{c}) if signature verification fails (as relayed by the \ecDataMod{}-module, and verified in a specialized circuit);
		otherwise it returns $\textbf{o} \in \mathbb{B}_{\evmWordSize}$ a $\evmWordSize$-byte slice
		$ \utt{00} \; \utt{00} \; \cdots \; \utt{01} \in \mathbb{B}_{\evmWordSize}$.
		% this can fail both for reasons known to the \hubMod{} module and for reasons known to \textsc{ram};
		% (\emph{a}) whenever $\CDS \neq 160$
		% (\emph{b}) insufficient gas
		% (\emph{c}) malformed data;
