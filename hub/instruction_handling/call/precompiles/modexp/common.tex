...
\begin{description}
	\item[\underline{\underline{Miscellaneous row $n^°(i +  \prcModexpFirstXbsRowOffset)$: \locBbs{} extraction and analysis row:}}]
		we impose
		\begin{description}
			\item[\underline{Setting lookup flags:}]
				we impose
		...
		\end{description}
	\item[\underline{\underline{Miscellaneous row $n^°(i +  \prcModexpSecondXbsRowOffset)$: \locEbs{} extraction and analysis row:}}] we impose
		\begin{description}
			\item[\underline{Setting lookup flags:}]
				we impose
				\[
		...
		\end{description}
	\item[\underline{\underline{Miscellaneous row $n^°(i +  \prcModexpThirdXbsRowOffset)$: \locMbs{} extraction and analysis row:}}] we impose
		\begin{description}
			\item[\underline{Setting lookup flags:}]
				we impose
				\[
			...
			\item[\underline{Setting \oobMod{} values and defining shorthands:}]
				...
				\[
					...
					\end{array} \right]
					% QUESTION: what if we don't extract xbs ? Does the above work ?
					% ANSWER: yes: the OOB xbs-analysis rows are active no matter what.
					% \left[ \begin{array}{cl}
					% 	\cdot & \left[ \begin{array}{crcl} + & \locExtractBbs & \cdot & \locBbsIsWithinBounds \\ + & [1 - \locExtractBbs] \\ \end{array} \right] \vspace{2mm} \\
					% 	\cdot & \left[ \begin{array}{crcl} + & \locExtractEbs & \cdot & \locEbsIsWithinBounds \\ + & [1 - \locExtractEbs] \\ \end{array} \right] \vspace{2mm} \\
					% 	\cdot & \left[ \begin{array}{crcl} + & \locExtractMbs & \cdot & \locMbsIsWithinBounds \\ + & [1 - \locExtractMbs] \\ \end{array} \right] \\
					% \end{array} \right]
				\]
				\saNote{}
				The above definition of \locAllByteSizesAreInBounds{}
				is the correct one, and, in particular,
				is compatible with all scenarios
				where call data is \emph{very short}
				i.e. $\locPrcCds < \cumulativeLengthOfModexpByteSizes = 3 \cdot \evmWordSize$,
				the ``cumulative length of byte sizes''.
				Indeed, the \oobMod{}-calls analyzing
				the various \locXbs'
				are  triggered no matter what,
				and in particular: regardless of whether or not
				the \locPrcCds{} is big enough to warrant
				extracting the various byte sizes
				(\locBbs{}, \locEbs{} and \locMbs{})
				from \textsc{ram}.
				One may choose to impose the further sanity check constraint
				\[
					\underbrace{
						\begin{array}{l}
							\locAllByteSizesAreInBounds
							=
							\left[ \begin{array}{cl}
								\cdot & \left[ \begin{array}{crcl} + & \locBbsIsWithinBounds & \!\!\!\cdot\!\!\! & \locExtractBbs \\ + & \numConst{1} & \!\!\!\cdot\!\!\! &  \big[ 1 - \locExtractBbs \big] \\ \end{array} \right] \vspace{2mm} \\
									\cdot & \left[ \begin{array}{crcl} + & \locEbsIsWithinBounds & \!\!\!\cdot\!\!\! & \locExtractEbs \\ + & \numConst{1} & \!\!\!\cdot\!\!\! &  \big[ 1 - \locExtractEbs \big] \\ \end{array} \right] \vspace{2mm} \\
										\cdot & \left[ \begin{array}{crcl} + & \locMbsIsWithinBounds & \!\!\!\cdot\!\!\! & \locExtractMbs \\ + & \numConst{1} & \!\!\!\cdot\!\!\! &  \big[ 1 - \locExtractMbs \big] \\ \end{array} \right] \\
							\end{array} \right]
							\vspace{2mm}
						\end{array}
					}_{\displaystyle \sanityCheck}
				\]
				\saNote{}
		...
		\begin{description}
			\item[\underline{\underline{Miscellaneous row $n^°(i + \prcModexpLeadRowOffset)$: exponent leading word extraction and analysis row:}}] we impose
				\begin{description}
					\item[\underline{Setting lookup flags:}]
						...
						\]
						\saNote{}
						The shorthand \locExtractLeadingWord{} labeled with $\undefinedStar$ is defined below.
						...
						\[
							...
							\saNote{}
							\[
								\locCallOobForModexpForLeadWordExtractionAnalysis{}
					...
					\item[\underline{Setting some shorthands:}]
						...
						\[
							\left\{ \begin{array}{lcl}
								\locExtractLeadingWord & \define & \miscOobDataCol{4} _{i + \prcModexpLeadRowOffset} \\
						...
						\]
					\item[\underline{Setting \mmuMod{} values:}]
						\If $\locCallMmuForModexpToExtractLeadingWord = 1$ \Then we impose
						\[
					...
					\item[\underline{Setting some shorthands:}]
						...
						\]
						\saNote{}
						Again, section~(\ref{hub: miscellaneous constraints: automatic vanishing})
						imposes vanishing conditions in case the \mmuMod{} module isn't called.
					\item[\underline{Setting \expMod{} values:}]
						\If $\locCallExpForModexpToAnalyzeLeadingWord = 1$ \Then
						\[
					...
					\item[\underline{Setting some shorthands:}]
						...
						\saNote{}
						Again, section~(\ref{hub: miscellaneous constraints: automatic vanishing})
						imposes vanishing conditions in case the \expMod{} module isn't called.
				\end{description}
			\end{description}
			...
			\begin{description}
				\item[\underline{\underline{Miscellaneous row $n^°(i + \prcModexpPricingRowOffset)$: \instModexp{} pricing row:}}] we impose
					\begin{description}
						\item[\underline{Setting lookup flags:}]
							we impose
							\[
