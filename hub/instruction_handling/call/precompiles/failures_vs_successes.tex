...
\]
Notations are those of the \cite{EYP}.

\saNote{}
Thus an \inst{ECRECOVER} which is provided with (sufficient gas but) malformed data, preventing it from recovering an address, constitutes a ``precompile success'' (with $\textbf{o} = ()$),
while an \inst{ECADD} / \inst{ECMUL} / \inst{ECPAIRING} which is provided with (sufficient gas but) malformed data is considered a ``precompile failure'' (with $\textbf{o} = ()$.)
