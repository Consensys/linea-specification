...
\begin{description}
	...
	\item[\underline{Setting \stpMod{} instruction parameters:}]
		...
		\]
		\saNote{}
		\inst{CALL} is the only \inst{CALL}-type instruction which may incur the creation of an account.
		Recall that this happens whenever a \inst{CALL} targets an address for which no account exists in the state and transfers a nonzero value.
