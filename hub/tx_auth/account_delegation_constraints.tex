\begin{center}
	...
	\left\{ \begin{array}{lcl}
		\txAuth      _{i + \roffAuthorizationProcessingAccountRow} = 1 \\
		\peekAccount _{i + \roffAuthorizationProcessingAccountRow} = 1 \\
	\end{array} \right.}
\end{center}
We remind the reader of
constraint~(\ref{hub: authorization phase: perspectives: account row iff authority successfully ECRECOVER'ed}):
account-rows in the \txAuth-phase correspond precisely to authorization tuples with
$\authAuthorityEcrecoverSuccess \equiv \true$.

We define the following shorthands
\[
	\left\{ \begin{array}{lcl}
		\locAuthAuthorityAddressHi & \define & \authAuthorityAddressHi  _{i + (\roffAuthorizationProcessingAuthorityRow)} \\
		\locAuthAuthorityAddressLo & \define & \authAuthorityAddressLo  _{i + (\roffAuthorizationProcessingAuthorityRow)} \\
	\end{array} \right.
\]
...
\begin{enumerate}
	...
	\item we peek into the authority account
		\[
			\left\{ \begin{array}{lclr}
				\accAddress \high  _{i + \roffAuthorizationProcessingAccountRow} & = & \locAuthAuthorityAddressHi              \\
				\accAddress \low   _{i + \roffAuthorizationProcessingAccountRow} & = & \locAuthAuthorityAddressLo \vspace{2mm} \\
				\multicolumn{4}{l}{
					\accTrimAddress
					{i}{\roffAuthorizationProcessingAccountRow}
					{\locAuthAuthorityAddressHi}
					{\locAuthAuthorityAddressLo}
				} \vspace{2mm} \\
				...
				\multicolumn{3}{l}{\accTurnOnWarmth               {i}{\roffAuthorizationProcessingAccountRow}} \\
				...
				\multicolumn{3}{l}{
					\standardDomSubStamps {
						...
						domOffset = \roffAuthorizationProcessingAccountRow ,
					}
				} \\
			\end{array} \right.
		\]
	...
	\item[\underline{\underline{Triggering delegation detection i.e. the \romLexMod{} lookups:}}]
		...
		\[
			\accCheckForDelegationInAuthorizationPhase {
				anchorRow           = i                                      ,
				relOffset           = \roffAuthorizationProcessingAccountRow ,
				% checkPostDelegation = \locCheckPostDelegation                ,
			}
		\]
		% where
		% \[
		% 	\locCheckPostDelegation \define
		% 	\left[ \begin{array}{cl}
...
\begin{description}
	...
	\item[\underline{\underline{The valid authority tuple case:}}]
		\If $\locTupleValidity = \true$ \Then we impose
		\begin{description}
			\item[\underline{Common constraints:}]
				the following constraints hold in all cases:
				\[
					\left\{ \begin{array}{ll}
			...
			\item[\underline{Authority code hash update:}]
				...
				\begin{enumerate}
					\item \If $\locDelegationReset  = \true$ \Then
						\[
							\left\{ \begin{array}{lclr}
								\accHasCode    \new _{i + \roffAuthorizationProcessingAccountRow} & = & 0              & (\sanityCheck) \\
								\accCodehashHi \new _{i + \roffAuthorizationProcessingAccountRow} & = & \emptyKeccakHi \\
								\accCodehashLo \new _{i + \roffAuthorizationProcessingAccountRow} & = & \emptyKeccakLo \\
							\end{array} \right.
						\]
