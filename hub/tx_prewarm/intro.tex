...
see section~(\ref{rlp txn: phase constraints: access list}).
...
see section~(\ref{user txn data: processing: user: specialized computations}).

The structure of these pre-warming operations is simple.
Every access list item $E \equiv (E_\text{a}, E_\textbf{s})$,
see section~(\ref{rlp txn: phase constraints: access list}), contains an address $E_\text{a}$.
