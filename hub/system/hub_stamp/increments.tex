...
\begin{enumerate}
	\item the following specifies its increments:
		\begin{enumerate}
			...
			\item \label{hub: system: hub stamp: increments: AUTH case} \If $\txAuth _{i} = 1$ \Then $\hubStamp _{i + 1} = \hubStamp _{i} + \locTxAuthHubStampDelta _{i}$
		...
		where we defined
		\begin{enumerate}
			\item \If $\peekAuthorization _{i + 1} + \peekTransaction _{i + 1} + \txWarm _{i + 1} + \txSkip _{i + 1} + \txInit _{i + 1} =    0$ \Then $\locTxAuthHubStampDelta _{i} \define 0$
			\item \If $\peekAuthorization _{i + 1} + \peekTransaction _{i + 1} + \txWarm _{i + 1} + \txSkip _{i + 1} + \txInit _{i + 1} \neq 0$ \Then $\locTxAuthHubStampDelta _{i} \define 1$
		\end{enumerate}
\end{enumerate}
