...
\begin{enumerate}
	\item the following specifies its increments:
		\begin{enumerate}
			...
			\item \label{hub: system: hub stamp: increments: AUTH case} \If $\txAuth _{i} = 1$ \Then $\hubStamp _{i + 1} = \hubStamp _{i} + \peekAuthorization _{i + 1} + \peekTransaction _{i + 1} + \txSkip _{i + 1} + \txInit _{i + 1}$
