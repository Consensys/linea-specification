...
\begin{enumerate}
	\item the following specifies its increments:
		...
		where we defined
		\begin{enumerate}
		        \item \If $\peekAuthorization _{i + 1} + \peekTransaction _{i + 1} + \txSkip _{i + 1} + \txInit _{i + 1} =    0$ \Then $\locTxAuthHubStampDelta _{i} \define 0$
		        \item \If $\peekAuthorization _{i + 1} + \peekTransaction _{i + 1} + \txSkip _{i + 1} + \txInit _{i + 1} \neq 0$ \Then $\locTxAuthHubStampDelta _{i} \define 1$
		\end{enumerate}
\end{enumerate}
