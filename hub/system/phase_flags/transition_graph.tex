\begin{figure}[!h]
        \begin{tikzpicture}[node distance={4cm}, thick, main/.style={rectangle, thick, draw, rounded corners = 3pt, inner sep = 6pt, outer sep = 4pt}]
                \node[main, color = draculagreen]  (auth)                        {$\txAuth$};
                \node[main, color = draculagreen]  (warm) [right of=auth]        {$\txWarm$};
        ...
        \caption{%
                The above is the transition graph of the boolean flags $\txAuth$, $\txWarm$, $\txSkip$, $\txInit$, $\txExec$ and $\txFinl$.
