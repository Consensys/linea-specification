\item[\underline{\underline{Recipient account-row n$^°~\bm{(i + \roffTxSkipUserRecipientAccount)}$:}}]
	...
	\[
		\left\{ \begin{array}{lcl}
			...
			\multicolumn{3}{l}{\accConditionallyCheckForDelegation  { anchorRow = i, relOffset = \roffTxSkipUserRecipientAccount, condition = \locCheckRecipientForDelegation } } \\
	...
	where
	\[
		\locCheckRecipientForDelegation
		\define
		\left[ \begin{array}{cl}
			\cdot & \locIsMessageCallTransaction \\
			\cdot & \locRecipientHasNonemptyCode \\
		\end{array} \right]
	\]
	In order to correctly update the recipient account one must distinguish between
	message call transactions (e.g. pure transfers) and
	(trivial) deployments.
	...
	\begin{description}
		\item[\underline{Pure transfers:}]
			\If $\locIsMessageCallTransaction = 1$ \Then
			\[
				\left\{ \begin{array}{lcl}
		...
		\item[\underline{Trivial deployments:}]
			\If $\locIsDeploymentTransaction = 1$ \Then
			\begin{description}
				...
				\item[\underline{Deployment:}]
					...
					the initial (world) state is made to contain an account with nonempty bytecode which allows it to \inst{SELFDESTRUCT}.
