\item[\underline{\underline{Transaction-row n$^°~(\bm{i + \roffTxSkipUserTransaction})$:}}]
	...
	\begin{description}
		\item[\underline{(Partially) justifying \txRequiresEvmExecution{}:}]
			...
			\begin{enumerate}
				...
				\item
					...
					\[
						\left\{ \begin{array}{lcl}
							\locRecipientHasEmptyCode                         & \define & (1 - \accHasCode _{i + \roffTxSkipUserRecipientAccount})                                                                         \\
							\locRecipientIsDelegatedAndDelegationHasEmptyCode & \define & \accIsDelegated  _{i + \roffTxSkipUserRecipientAccount} \cdot (1 - \accHasCode _{i + \roffTxSkipUserRecipientOrDelegationAccount}) \\
							\locRecipientIsDelegatedAndDelegationIsDelegated  & \define & \accIsDelegated  _{i + \roffTxSkipUserRecipientAccount} \cdot \accIsDelegated  _{i + \roffTxSkipUserRecipientOrDelegationAccount} \\
						\end{array} \right.
					\]
				\item
		...
		\item[\underline{Justifying $\txFinalRefundCounter$:}]
			...
			\cite{EIP-7702}, transactions which require no code execution
			may still accrue refunds due to successful delegation operations.
