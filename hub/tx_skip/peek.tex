...
\begin{enumerate}
	...
	\]
\end{enumerate}
% \saNote{} The above enforces (through other constraints) that $\batchNum_{j}$, $\txNum_{j}$, $\hubStamp_{j}$ and $\txSkip_{j}$ (for $i\leq j \leq i+3$) remain constant throughout; furthermore, if the trace contains an $(i+4)$-th row, then at that row the pair $\big[ \batchNum, \txNum \big]$ changes and $\hubStamp$ jumps by $1$.
