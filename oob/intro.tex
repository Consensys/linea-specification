	...
	\item[\underline{\inst{ECRECOVER}, \inst{ECADD} and \inst{ECMUL} pricing:}]
		...
		\[
			\left\{ \begin{array}{llcrcl}
				\text{for }\inst{ECRECOVER}   : & \locCalleeGas \leq g_\text{r} & := & \locGasCostEcrecover \\
				\text{for }\inst{ECADD}       : & \locCalleeGas \leq g_\text{r} & := & \locGasCostEcadd     \\
				\text{for }\inst{ECMUL}       : & \locCalleeGas \leq g_\text{r} & := & \locGasCostEcmul     \\
			\end{array} \right.
		\]
	\item[\underline{\inst{SHA2-256}, \inst{RIPEMD-160} and \inst{IDENTITY} pricing:}]
		...
		\[
			\left\{ \begin{array}{llcrcr}
				\text{for }\inst{SHA2-256}   : & \locCalleeGas \leq g_\text{r} & := & \locGasCostShaTwo   & + & \locGasCostShaTwoWord   \cdot \locCeil  \\
				\text{for }\inst{RIPEMD-160} : & \locCalleeGas \leq g_\text{r} & := & \locGasCostRipemd   & + & \locGasCostRipemdWord   \cdot \locCeil  \\
				\text{for }\inst{IDENTITY}   : & \locCalleeGas \leq g_\text{r} & := & \locGasCostIdentity & + & \locGasCostIdentityWord \cdot \locCeil  \\
			\end{array} \right.
		\]
	\item[\underline{\inst{ECPAIRING} pricing:}]
		verifies that $\| I_\textbf{d} \| \equiv 0 \mod \locPairingSize$ and performs the comparison 
		\[
			\locCalleeGas \leq
			g_\text{r} :=
			\locGasCostPairingBase +
			\locGasCostPairingPair \cdot \frac{\| I_\textbf{d} \|}{\locPairingSize}
		\]
\end{description}
