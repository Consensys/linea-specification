...
\begin{description}
	...
	\item[\underline{\maxcsxSH{} detection:}]
		determine if a \inst{RETURN} in a deployment context returns code that is too long (i.e. $(\col{codesize} > \locMaxCodeSizeValue$);
		see section~(\ref{oob: populating: return});
		alternatively determine if a \inst{CREATE(2)} instruction's init code is too long (i.e. $(\col{initcodesize} > \locMaxInitCodeSizeValue$);
\end{description}
