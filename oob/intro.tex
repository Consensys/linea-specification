...
What follows is the list of tasks in which the present module helps the \hubMod{}\footnote{These are deliberately vague, full details are in section~(\ref{oob: populating: opcodes})}:
\begin{description}
	\item[\underline{Jump-destination pre-vetting:}]
		...
		see section~(\ref{oob: populating: opcodes: jump}) and section~(\ref{oob: populating: opcodes: jumpi});
	\item[\underline{\rdcxSH{} detection:}]
		...
		\]
		or not;
		see section~(\ref{oob: populating: opcodes: rdc});
	\item[\underline{Trivial \inst{CALLDATALOAD} detection:}]
		...
		see section~(\ref{oob: populating: opcodes: cdl});
	...
	\item[\underline{\inst{CALL}-type abort detection:}]
		...
		see section~(\ref{oob: populating: opcodes: call});
	...
	\item[\underline{\sstorexSH{} detection:}]
		...
		see section~(\ref{oob: populating: opcodes: sstore});
	\item[\underline{\maxcsxSH{} detection:}]
		...
		see section~(\ref{oob: populating: opcodes: deployment});
\end{description}
...
\begin{description}
		\item[]
			\underline{\instEcrecover{}, \instShaTwo{}, \instRipemd{}, \instIdentity{},}\\
			\underline{\instEcadd{}, \instEcmul{}, \instEcpairing{}, \instPointEvaluation{},}\\
			\underline{\instBlsGOneAdd{}, \instBlsGTwoAdd{}, \instBlsMapFpToGOne{},}\\
			\underline{\instBlsMapFpTwoToGTwo{}:}
			detect both $\cds \equiv 0$ and $\rac \equiv 0$;
		\item[]
			\underline{\instEcrecover{}, \instEcadd{}, \instEcmul{} and}\\
			\underline{\instPointEvaluation{} pricing:}
	...
	\item[\underline{\instBlsGOneAdd{},
		...
		\[
			...
			\end{array} \right.
		\]
		See section~(\ref{oob: precompiles: bls: fixed size and cost}) for the definition of \locFixedCds{}.	
	...
	\item[\underline{\instBlsGOneMsm{} and \instBlsGTwoMsm{}:}]
		verifies that $\| I_\textbf{d} \| \equiv 0 \mod \locMinMsmSize$ and performs the comparison 
			\[ \resizebox{.95\linewidth}{!}{$\locCalleeGas \leq g_\text{r} := \frac{\| I_\textbf{d} \|}{\locMinMsmSize} \cdot \locMsmMultiplicationCost \cdot \frac{\locDiscount}{\prcBlsMultiplicationMultiplier}$} \]
			See section~(\ref{oob: precompiles: bls: msm}) for the definition of \locMinMsmSize{}, \locMsmMultiplicationCost{}, \prcBlsMultiplicationMultiplier{} and \locDiscount{}.
