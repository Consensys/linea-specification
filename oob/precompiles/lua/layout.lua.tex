...
|                            | ✓ trusted  | ✓ trusted  | ✓ trusted     | ⟦ π ⟧                     | ⟦ π ⟧                            | ⟦ π ⟧                      | ⟦ π ⟧                      | ⟦ π ⟧                      |                     |                     |
|----------------------------+------------+------------+---------------+---------------------------+----------------------------------+----------------------------+----------------------------+----------------------------+---------------------+---------------------|
| ⟦OOB_INST_p256_verify⟧     | callee_gas | cds        | r@c           | hub_success ≡             | return_gas ≡                     | extract_call_data ≡        | dont_extract_call_data ≡   | r@c_nonzero ≡              |                     |                     |
...
- ⟦OOB_INST_p256_verify⟧ is ALMOST identical, in terms of its column specification, to the
  ⟦OOB_INST_pointevaluation⟧ instruction and the ⟦OOB_INST_bls_*⟧ instructions. There is,
  however, one important difference: for the ⟦OOB_INST_pointevaluation⟧ and the ⟦OOB_INST_bls_*⟧
  it holds that:
...
- The ONLY scenario under which a call to the P256_VERIFY precompile may fail is if it isn't given enough gas.
  Its call data size is wrong, or its call data being malformed only impacts its return data, but not the
