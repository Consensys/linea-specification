...
\begin{description}
    ...
    % \item[Preprocessing row $n^\circ (i + \rowNum)$:] \decisionRequired{}
    %     we impose
    %     \[
    %         \callToIszero
    %         {i}{\rowNum}
    ...
    \item[Preprocessing row $n^\circ (i + \rowNum)$:]
        ...
        \]
\end{description}
% \saNote{}\label{exp: modexp log: preprocessing: trivial trim interpretation} By construction one has that
% \begin{IEEEeqnarray*}{LCL}
%     \locTrivialTrim = 1 & \iff & \locPaddedLogBaseTwo = 0     \\
%                         & \iff & \locTrimAcc < \in \{ 0, 1 \} \\
% \end{IEEEeqnarray*}
