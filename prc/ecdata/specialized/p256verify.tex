...
\begin{description}
    ...
    \item[\underline{Setting / sanity checking the result:}]
        ...
        \]
\saNote{}
The above constraining of the result is a sanity check in case $\csPVerify_{i} \equiv \true$, since in that case an external circuit is already tasked with justifying both the result and the success bit. However, it is necessary to set the result in case $\csPVerify_{i} \equiv \false$ since no call to external circuits is made in that case.  
\end{description}
