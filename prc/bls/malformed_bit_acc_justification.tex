The intent with the \malformedDataBit{} column is that this column may only turn on for a single \textbf{large} point within a given call to the \inst{PAIRING\_CHECK} precompile. Furthermore it should only be able to turn on if internal checks of the module passed.
