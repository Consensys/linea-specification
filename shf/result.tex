In this subsection we specify the bytes of the result columns $\resHi$ and $\resLo$.
Recall that given the target constraints from subsection~\ref{shf: target}, these are recorded in the $\byteCol{4}$ and $\byteCol{5}$ byte columns.
...
%\paragraph{$\resHi$ and $\resLo$ when $\known = 1$:} ---
...
%			\resLo_{i} = 0 \\
		...
		\left\{
		\begin{array}{l}
%			\resHi_{i} = \NEG_{i} \cdot \left( 256^{\llarge} - 1 \right) \\
%			\resLo_{i} = \NEG_{i} \cdot \left( 256^{\llarge} - 1 \right) \\
