	...
	\item
		...
		\[
			\left\{ \begin{array}{lcll}
				\rlpTxnSharedColumnIsPrefixOfAccessListItem _{i} & = & \false & (\sanityCheck), \text{ see section~(\ref{rlp txn: phase constraints: access list: access list bits})} \\
...
\begin{enumerate}[resume]
	\item \If $\phaseEnd _{i} = \false$ \Then:
		...
		section~(\ref{rlp txn: phase constraints: access list: access list bits}) and
		constraint~(\ref{rlp txn: phase constraints: access list: legal transitions: start with access list item prefix}).
\end{enumerate}
