...
As such it ought to remain constant along the rows pertaining to a given authorization tuple.
Tuple indexing will start at $1$ and go all the way up to $\rlpTxnCommonColumnNumberOfAuthorizations$.
Processing of a given tuple will always occupy $\locAuthorizationNumberOfRows$ rows.
Authorization validity will be checked in the \rlpAuthMod{} module.
...
\[
	\locAuthorizationIndex
	~ \equiv ~
	\overbrace{\leftUndefined}^{\displaystyle \isTxn\text{-row}} , 0, ~
...
\begin{enumerate}
	...
	\item\label{rlp txn: phase constraints: authorization list: constraints: monotonicity of tuple index}
		\If   $\locAuthorizationAgainAuthPhase  _{i} = \true$
		\Then
		\[
			\locAuthorizationIndex _{i} \in \{ \locAuthorizationIndex _{i - 1}, 1 + \locAuthorizationIndex _{i - 1} \}
		\]
	\item \If $\isCmp_{i} = 1$ \et $\locAuthorizationIndex _{i} \neq \rlpTxnCommonColumnNumberOfAuthorizations _{i}$
		\Then
		\[
