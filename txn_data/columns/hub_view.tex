...
\begin{enumerate}
	...
	\item
		\markAsJustifiedHere{}
		$\txnDataHubTransactionTypeSupportsEipFifteenFiftyNineGasSemantics$:
		boolean column;
		lights up for transactions implementing \cite{EIP-1559} gas semantics;
	\item
		\markAsJustifiedHere{}
		$\txnDataHubTransactionTypeSupportsDelegationLists$:
		boolean column;
		lights up for transactions supporting delegation lists;
	\item
	...
	\item
		\markAsExtractedFromHub{}
		$\txnDataHubLengthOfDelegationList$:
		counts the number of items in the delegation list
		for \user{}-transaction whose underlying transaction type supports
		\texttt{set code transactions};
	\item
		\markAsExtractedFromHub{}
		$\txnDataHubNumberOfSuccessfulSenderDelegations$:
		counts the number of authorization tuples $\loc{tuple}$ that are both
		(\emph{a}) valid and
		(\emph{b}) have $\loc{tuple.authority()} \equiv \loc{tx.from()}$;
		for \user{}-transaction whose underlying transaction type supports
		\texttt{set code transactions};
\end{enumerate}
