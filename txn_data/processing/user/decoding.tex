	...
	\begin{enumerate}
		...
		\item \label{user txn data: processing: user: transaction decoding: no access list if tx doesn't allow it}
			...
			\[
				...
				\end{array} \right.
			\]
		\item \label{user txn data: processing: user: transaction decoding: no account delegations if tx doesn't allow it} \If $\locTransactionSansDelegation = 1$ \Then $\locRlpNumAccountDelegations = 0$ (\sanityCheck)
	\end{enumerate}
