...
\begin{enumerate}
	...
	\item $\txnDataMod{}$:
		...
		%%%
	\item $\btcMod{}$:
		this module corrals some (\textsc{evm}-relevant) batch data (such as \inst{COINBASE}, \inst{TIMESTAMP}, \dots);
	...
	\item $\mxpMod{}$:
		...
		%%%
	\item $\romMod{}$:
		contains the bytecodes (as well as initialization codes) which are run, read (e.g. through \inst{EXTCODECOPY}) and or (temporarily) deployed in a batch of transactions;
