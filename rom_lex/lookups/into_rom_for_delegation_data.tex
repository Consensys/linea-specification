	...
	\item[\underline{Target columns:}] from the \romMod{} module: 
		\begin{multicols}{2}
			\begin{enumerate}
				...
				\item $\cfi                      _{j}$
				\item[\vspace{\fill}]
				\item $\loc{prefix\_sum}         _{j}$
				\item $\loc{4\_leading\_bytes}   _{j}$
				\item $\loc{16\_trailing\_bytes} _{j}$
			\end{enumerate} 
		\end{multicols}
		where we use the following shorthands:
		\[
			\left\{ \begin{array}{lcl}
				\loc{prefix\_sum}         _{j} & = & \displaystyle \sum _{k = 0} ^ {2}  256 ^ {2  - k} \cdot \PBCB _{j + k} \\
				\loc{4\_leading\_bytes}   _{j} & = & \displaystyle \sum _{k = 0} ^ {3}  256 ^ {3  - k} \cdot \PBCB _{j + 3  + k} \\
				\loc{16\_trailing\_bytes} _{j} & = & \displaystyle \sum _{k = 0} ^ {15} 256 ^ {15 - k} \cdot \PBCB _{j + 7  + k} \\
			\end{array} \right.
		\]
\end{description}
