...
# New lookups `HUB → ROM_LEX`

```rust
selector ≡ PEEK_AT_ACCOUNT
           ∧ [acc/CODE_SIZE == 23]
           ∧ ¬acc/DEPLOYMENT_STATUS
...
CODE_HASH_HI
CODE_HASH_LO
DELEGATION_ADDRESS_HI
DELEGATION_ADDRESS_LO
```

We must have a similar lookup for the updated side of things:

```rust
selector ≡ PEEK_AT_ACCOUNT
           ∧ [acc/CODE_SIZE_NEW == 23]
           ∧ ¬acc/DEPLOYMENT_STATUS_NEW
...
DELEGATION_NUMBER_NEW
CODE_SIZE_NEW
CODE_HASH_HI_NEW
CODE_HASH_LO_NEW
DELEGATION_ADDRESS_HI_NEW
DELEGATION_ADDRESS_LO_NEW
```

# New reason to trigger `ROM_LEX`

With type 1 around the corner (so to speak ...) we will have to further justify data, specifically the code size. The issue arises, AFAICT, only with `EXTCODESIZE`. `EXTCODEHASH` directly queries the state, and when doing `EXTCODECOPY` we are already forced to `trigger_ROM_LEX`, thus hash the bytecode and confront the result to the account's codehash. The `EXTCODESIZE` must be explictly linked to the code hash, which is the only trace of the byte code in the Ethereum state. Thus we must explicitly trigger the `ROM_LEX` module for successful `EXTCODESIZE` calls (to accounts not currently undergoing deployment).
