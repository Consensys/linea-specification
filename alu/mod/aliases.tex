For the sake of clarity and connecting the constraints of this section with the strategy for proving a euclidean division outlined in section~(\ref{alu: mod: target}) we introduce some \emph{aliases} for certain \emph{values}. We emphasize that these don't represent new columns. They are only meant to capture the value of the accumulators at the final row of the byte accumulation process i.e. rows with $\ct_{i} = \mmediumMO$. They thus represent the target values these accumulators strive for.
\begin{multicols}{3}
\begin{enumerate}
