...
it follows that
$\argTwo \geq 256$ \emph{iff} one of the $\mmedium$-byte integers $\col{B\_3}$, $\col{B\_2}$, $\col{B\_1}$ is $\neq 0$ or they are all $=0$ so that $\argTwo = \col{B\_0}$ but (the $\mmedium$-byte integer) $\col{B\_0}$ isn't a byte i.e. (since $\byteCol{B\_0}$ contains its byte decomposition by virtue of section~\ref{alu: mul: byte decompositions})
