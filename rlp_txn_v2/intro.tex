...
First of all we recall that, in the \zkEvm{} in general, and in this module in particular, transactions are identified with their absolute transaction number $\userTransactionNumber$.
The reader will find more information on this in the \userTxnDataMod{} module.
We therefore consider a (well-formed) transaction $\locTransaction$ with $\userTransactionNumber \equiv \col{a}$.
The goal is that, considering exactly those rows $i$ where
\begin{multicols}{3}
	\begin{itemize}
		\item $\userTransactionNumber_{i} \equiv \col{a}$
		\item $\lc_{i} \equiv 1$
		\item $\lt_{i} \equiv 1$
	\end{itemize}
\end{multicols}
